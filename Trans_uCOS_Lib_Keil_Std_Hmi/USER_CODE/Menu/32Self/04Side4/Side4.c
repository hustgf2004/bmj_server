...
{
    pmiiThis->pvPara = (void *)&GpuiValveTbl[SIDE_4_OUT];
    return SYS_OK;
}
...
{    
    ...
    if (GpuiKeyTbl[KEY_AUX_LEFT] == uiKeyCode) {                    /*  辅助左键                    */
        Gds.uiValveStatus |= GpuiValveTbl[SIDE_4_OUT];
    } else if (GpuiKeyTbl[KEY_AUX_RIGHT] == uiKeyCode) {            /*  辅助右键                        */
        Gds.uiValveStatus |= GpuiValveTbl[SIDE_4_IN];
    } else {                                                            /*  其他                        */
        Gds.uiValveStatus &= ~(GpuiValveTbl[SIDE_4_OUT] | GpuiValveTbl[SIDE_4_IN]);
    }
#endif
...
const MENU_ITEM_FUNC GmifSide4 = {
    __menuItemCreate,
    __menuItemDel,
    __menuItemOnKey,
    __menuItemParamFmt
...
/*********************************************************************************************************
  辅助动作侧四菜单项定义
*********************************************************************************************************/
const MENU_ITEM_INFO_BASE __GmiibSide4 = {                             /*  菜单项信息                  */
    "侧四伸  侧四收",
    (MENU_ITEM_FUNC *)&GmifSide4,
    0x3204,    
};
