...
{
    pmiiThis->pvPara = (void *)&GpuiValveTbl[SIDE_1_OUT];
    return SYS_OK;
}
...
{    
    ...
    if (GpuiKeyTbl[KEY_AUX_LEFT] == uiKeyCode) {                    /*  辅助左键                    */
        Gds.uiValveStatus |= GpuiValveTbl[SIDE_1_OUT];
    } else if (GpuiKeyTbl[KEY_AUX_RIGHT] == uiKeyCode) {            /*  辅助右键                        */
        Gds.uiValveStatus |= GpuiValveTbl[SIDE_1_IN];
    } else {                                                            /*  其他                        */
        Gds.uiValveStatus &= ~(GpuiValveTbl[SIDE_1_OUT] | GpuiValveTbl[SIDE_1_IN]);
    }
#endif
...
const MENU_ITEM_FUNC GmifSide1 = {
    __menuItemCreate,
    __menuItemDel,
    __menuItemOnKey,
    __menuItemParamFmt
...
/*********************************************************************************************************
  辅助动作反冲洗菜单项定义
*********************************************************************************************************/
const MENU_ITEM_INFO_BASE __GmiibSide1 = {                             /*  菜单项信息                  */
    "侧一伸  侧一收",
    (MENU_ITEM_FUNC *)&GmifSide1,
    0x3201,    
};
