...
**
**--------------File Info---------------------------------------------------------------------------------
** File name:               LowPressureEn.c
** Latest modified Date:    2017-04-03
** Latest Version:          1.0
** Descriptions:            低压传感器使能菜单项
**
**--------------------------------------------------------------------------------------------------------
** Created by:              
...
extern const MENU_ITEM_INFO_BASE __GmiibPressureSnr;
...
/*********************************************************************************************************
  内部使用变量
*********************************************************************************************************/
const MENU_ITEM_FUNC GmifLowPressureEn = {
    __menuItemCreate,
    __menuItemDel,
    __menuItemOnKey,
    __menuItemParamFmt
...
{    
    ...
    */
    ...
    switch(puiKeyCode[0]) {
        ...
        case KEY_ENTER:
            if (!__GucEntered) {
                __SucEnFlag = (((INT32U *)pmiiThis->pvPara)[0] >> EN_LOW_P_SNR) & 0x01; 
                guiPrintf(uiX, uiY, 1, "?");
                break;
            }
            if (__SucEnFlag) {
                ((INT32U *)pmiiThis->pvPara)[0] |= (1 << EN_LOW_P_SNR);
            } else {
                ((INT32U *)pmiiThis->pvPara)[0] &= ~(1 << EN_LOW_P_SNR);
            }                
            //GdiDevInfo.ucPressureEn[0] = __SucEnFlag;
            ...
            if (__SucEnFlag) {
                ;//miAdd(&__GmiibPressureSnr, &__GmiibLowPressureRange);
            } else {
                ;//miDel(&__GmiibLowPressureRange);
            }
        break;
        case KEY_ESC:
            pmiiThis->pmiib->pmifFunc->pfuncParamFmt(pmiiThis, scParamFmt);
...
INT32S __menuItemParamFmt(MENU_ITEM_INFO *pmiiThis, void *pvRsv)
{
    ...
    */
    ...
    sprintf(pvRsv, ":%s", __SscEnStr[(ucPressureEn >> EN_LOW_P_SNR) & 0x01]);
    return SYS_OK;
}
...
/*********************************************************************************************************
  低压传感器使能菜单项定义
*********************************************************************************************************/
const MENU_ITEM_INFO_BASE __GmiibLowPressureEn = {                      /*  菜单项信息                  */
    "低压瓶压力",
    (MENU_ITEM_FUNC *)&GmifLowPressureEn,
    0x0801
};
