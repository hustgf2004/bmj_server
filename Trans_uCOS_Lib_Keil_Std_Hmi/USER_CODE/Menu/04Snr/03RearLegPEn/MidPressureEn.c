...
**
**--------------File Info---------------------------------------------------------------------------------
** File name:               MidPressureEn.c
** Latest modified Date:    2017-04-03
** Latest Version:          1.0
** Descriptions:            中压传感器使能菜单项
**
**--------------------------------------------------------------------------------------------------------
** Created by:              
...
/*********************************************************************************************************
  内部使用变量
*********************************************************************************************************/
const MENU_ITEM_FUNC GmifMidPressureEn = {
    __menuItemCreate,
    __menuItemDel,
    __menuItemOnKey,
    __menuItemParamFmt
...
{    
    ...
    */
    ...
    switch(puiKeyCode[0]) {
        ...
        case KEY_ENTER:
            if (!__GucEntered) {
                __SucEnFlag = (((INT32U *)pmiiThis->pvPara)[0] >> EN_MID_P_SNR) & 0x01; 
                guiPrintf(uiX, uiY, 1, "?");
                break;
            }
            if (__SucEnFlag) {
                ((INT32U *)pmiiThis->pvPara)[0] |= (1 << EN_MID_P_SNR);
            } else {
                ((INT32U *)pmiiThis->pvPara)[0] &= ~(1 << EN_MID_P_SNR);
            }
            //GdiDevInfo.ucPressureEn[1] = __SucEnFlag;
...
INT32S __menuItemParamFmt(MENU_ITEM_INFO *pmiiThis, void *pvRsv)
{
    ...
    */
    ...
    sprintf(pvRsv, ":%s", __SscEnStr[(ucPressureEn >> EN_MID_P_SNR) & 0x01]);
    return SYS_OK;
}
...
/*********************************************************************************************************
  中压传感器使能菜单项定义
*********************************************************************************************************/
const MENU_ITEM_INFO_BASE __GmiibMidPressureEn = {                      /*  菜单项信息                  */
    "中压瓶压力",
    (MENU_ITEM_FUNC *)&GmifMidPressureEn,
    0x0803
};
