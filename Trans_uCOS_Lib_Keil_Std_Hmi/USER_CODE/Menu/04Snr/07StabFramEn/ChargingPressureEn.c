...
**
**--------------File Info---------------------------------------------------------------------------------
** File name:               ChargingPressureEn.c
** Latest modified Date:    2017-04-03
** Latest Version:          1.0
** Descriptions:            加注压力传感器使能菜单项
**
**--------------------------------------------------------------------------------------------------------
** Created by:              
...
/*********************************************************************************************************
  内部使用变量
*********************************************************************************************************/
const MENU_ITEM_FUNC GmifChargingPressureEn = {
    __menuItemCreate,
    __menuItemDel,
    __menuItemOnKey,
    __menuItemParamFmt
...
{    
    ...
    */
    ...
    switch(puiKeyCode[0]) {
        ...
        case KEY_ENTER:
            if (!__GucEntered) {
                __SucEnFlag = (((INT32U *)pmiiThis->pvPara)[0] >> EN_CHARGING_P_SNR) & 0x01; 
                guiPrintf(uiX, uiY, 1, "?");
                break;
            }
            if (__SucEnFlag) {
                ((INT32U *)pmiiThis->pvPara)[0] |= (1 << EN_CHARGING_P_SNR);
            } else {
                ((INT32U *)pmiiThis->pvPara)[0] &= ~(1 << EN_CHARGING_P_SNR);
            }
            //GdiDevInfo.ucPressureEn[3] = __SucEnFlag;
...
INT32S __menuItemParamFmt(MENU_ITEM_INFO *pmiiThis, void *pvRsv)
{
    ...
    */
    ...
    sprintf(pvRsv, ":%s", __SscEnStr[(ucPressureEn >> EN_CHARGING_P_SNR) & 0x01]);
    return SYS_OK;
}
...
/*********************************************************************************************************
  加注压力传感器使能菜单项定义
*********************************************************************************************************/
const MENU_ITEM_INFO_BASE __GmiibChargingPressureEn = {                 /*  菜单项信息                  */
    "加注口压力",
    (MENU_ITEM_FUNC *)&GmifChargingPressureEn,
    0x0807
};
